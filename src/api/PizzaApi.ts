...
class PizzaApi {
  ...
  public async getTotals( errorHandler?: ( error: ApiError ) => void ): Promise<PizzaTotals> {
    return (
      this.handleResponse( await baseFetch<PizzaTotals>( `/totals/2020` ), errorHandler )
      || { costs: 0, donors: 0, locations: 0, meals: 0, orders: 0, pizzas: 0, raised: 0, states: 0 }
    );
  }
  ...
  public async getTrucks( showHistorical: boolean = false, locationId?: LocationId, errorHandler?: ( error: ApiError ) => void ): Promise<TruckQueryResults> {
    const result = await baseFetch<TruckQueryResults>( `/trucks` + toQueryString( {
      location: locationId,
